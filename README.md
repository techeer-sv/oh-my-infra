...
Docker Compose 기반의 통합 관측성(Observability) 스택입니다. 메트릭, 로그, 트레이싱, 프로파일링, 시각화를 독립적인 모듈로 구성하고, 외부 Docker 네트워크로 서비스 간 통신합니다.
...
```
  ...
  └─────────────────────────────────────────────────────┘
        ...
        ▼                  ▼                  ▼                           ▼
┌──────────────┐  ┌──────────────────┐  ┌──────────────┐       ┌─────────────────────┐
│  Prometheus  │  │loki-query-       │  │ jaeger-query │       │  pyroscope-         │
│  (metrics)   │  │frontend (:3100)  │  │  (:16686)    │       │  query-frontend     │
└──────┬───────┘  └────────┬─────────┘  └───────┬──────┘       │  (:4041)            │
       │                   │                    │              └──────────┬──────────┘
       │          ┌────────▼──────────┐         │                         │
       │          │loki-query-        │  ┌──────▼─────────┐      ┌────────▼──────────┐
       │          │scheduler          │  │otel-collector  │      │pyroscope-query-   │
       │          └────────┬──────────┘  │(:4317 gRPC)    │      │scheduler          │
       │                   │             └───────┬────────┘      └───────────┬───────┘
       │          ┌────────┴──────────┐          │                           │ 폴링
       │          ▼                   ▼          ▼                  ┌────────▼──────────┐
       │    ┌──────────┐      ┌────────────┐  ┌──────────────────┐  │pyroscope-querier  │
       │    │loki-read │      │loki-backend│  │jaeger-collector  │  └────┬────────┬─────┘
       │    └────┬─────┘      └──────┬─────┘  └────────┬─────────┘       │        │
       │         └─────────┬─────────┘                 │                 ▼        ▼
       │                   ▼                     ┌─────▼──────┐      ┌──────┐ ┌──────────────┐
       │             ┌──────────┐                │  ScyllaDB  │      │ingest│ │store-gateway │
       │             │  MinIO   │                └────────────┘      │er    │ │(MinIO)       │
       │             │  (Loki)  │                                    └──────┘ └──────────────┘
       │             └──────────┘
       │                   ▲
