...
| loki-ui | Loki | `http://loki-ui:3100` |
| jaeger | Jaeger | `http://jaeger-query:16686` |
| pyroscope | Pyroscope | `http://pyroscope-query-frontend:4040` |

> 각 데이터소스는 해당 스택(`make metrics`, `make logging`, `make tracing`, `make profiling`)이 먼저 실행되어 있어야 정상 연결됩니다. `loki-ui`는 loki-operational 플러그인에서 사용합니다.
