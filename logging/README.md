...
#### `loki-query-scheduler` — 쿼리 큐 관리

query-frontend와 loki-querier 사이의 중간 레이어입니다.
...
#### `loki-query-frontend` — 쿼리 API 진입점
...
└────┬────┘  └──────┬─────┘
  ...
  └───────┬────────┘
          ...
          ▼
  ┌─────────────┐
  │loki-querier │  Querier: 실제 데이터 조회
  └──────┬──────┘
       │                     ┌──────────────┐
